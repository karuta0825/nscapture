...
type PropsType = {
  ...
  files: any,
  refreshList: () => void,
  selectVideo: (order: number) => void,
  selectedItemNum: number | null
};

export default function PlayList(props: PropsType): React.Node {
  const {
    classs,
    files,
    refreshList,
  ...
  return (
    <div className={styles[classs]}>
      <div className={styles.header}>
        ...
        <IconButton
          className={styles.header__icon_update}
          color="primary"
          component="span"
          onClick={refreshList}
        >
      ...
      <div className={styles.body}>
        {
          files.map((v, k) => (
            <div
              className={(k === selectedItemNum) ? styles.item_selected : styles.item}
              onClick={() => selectVideo(k)}
            >
